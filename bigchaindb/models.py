...
from bigchaindb.fsm_validation import fsm
    ...
    def validate(self, bigchain, current_transactions=[]):
        ...
        if self.operation == Transaction.CREATE:
            ...
            if not fsm.verify_transition(self.asset):
                raise ValidationError('Transition from state `{}` to state `{}` with the given inputs is not possible, therefore the transaction is invalid.'.format(self.asset["data"]["from"], self.asset["data"]["to"]))
